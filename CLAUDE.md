...
**`python` *is* available (3.12.10) — but `python3` is not.** `python3` hits the Windows Store stub and fails; the working interpreter is plain `python`, at `~/AppData/Local/Programs/Python/Python312/`. This matters for editing: precise multi-line replacements in these large UTF-8 Arabic files are far safer through a short Python script (read → `str.replace` with an `assert` → write back with `encoding="utf-8"`) than through `sed`/`perl`, both of which have mangled these files before. Note Python resolves Windows paths, not Git Bash's `/tmp` — write temp files to the scratchpad's real path.
...
Backups from past risky edits are kept beside the originals, named for the change that prompted them rather than a version: `*.bak`, plus `*.colorbak` (the project-wide green text sweep), `*.mapbak` (adding the map watermark), `*.authbak` (wiring in Supabase auth — 9 files), `*.desktopbak` (adding the desktop layer), `*.publicbak` (opening public browsing), `account.html.fcbak` (deleting the factory card), `schema.sql.prefixbak`, and `i18n.js.b2bbak` / `i18n.js.msgbak` / `i18n.js.dashbak` (30-block key insertions). More recent: `desktop.css.wmbak` / `.wmbak2` (the wordmark animation, before and mid-rewrite), `.catbak` (the mega-menu anchoring), `.rowbak` (hiding the duplicate settings row), `*.i18nbak` (wiring the sidebar to i18n), and `CLAUDE.md.initbak`. They are the only "history" this folder has — don't delete them, and keep making one before any bulk rewrite. Note they are **point-in-time and not layered**: `welcome.html.bak` predates the tagline removal, so restoring it brings back markup the owner has since asked to delete. Read a backup before restoring it.
...
- `welcome.html` — **white** background (not green like `index.html`): the wordmark and the two entry actions "تسجيل دخول" (`login.html`) and "حساب جديد" (`register.html`), both layers animating in via staggered `animation-delay`. No auto-redirect — this is where the 5s splash timer lands and waits for user choice.
  ...
  - This page **used to** host an animated twelve-factory SVG scene (hand-written paths, rising smoke, `viewBox`-based scaling). It was deleted wholesale — the wordmark placed over it was unreadable. Along with it went `.factory-scene`, `.factory-illustration`, `.smoke`, `@keyframes sceneIn`, and `@keyframes smokeRise`. `welcome.html.bak` has the old version if it's ever wanted back; don't reconstruct it from scratch.
...
- Bottom-sheet modals are re-centered as ordinary dialogs on desktop, and `.categories-drawer` / `.drawer-overlay` are hidden — the `.dt-catbar` mega-menu replaces the drawer above 1024px.
  - **`.dt-catpanel` uses `left: 0` in *both* directions — this is not an oversight, and the `html[dir="ltr"]` rule deliberately overrides only `flex-direction`.** The panel is 780px wide and grows rightward from whichever edge anchors it, so `right: 0` pushes it off-screen and it gets clipped. That happens in *both* languages, for opposite reasons: in RTL the `.dt-catmenu` button sits at the far right, in LTR at the far left (it is the first child of a `row-reverse` flex row). Mirroring the anchor the way an asymmetric layout normally demands is exactly the bug — it shipped once, and the owner caught it with a screenshot after switching to English.
...
- **Coverage tiers** — this is a deliberate, disclosed scope trade-off, not an oversight:
  ...
  - The account-page language picker is a `[arabicName, nativeName, code]` triple array declared inline in `account.html` (`var languages`, ~line 679) with **90 entries** — it is *not* exported from `i18n.js`, so adding a language means touching both files. All 90 have real ISO-ish codes and perform a real switch; the 60 outside the 30-language tier fall back to English UI text via the `t()` fallback chain rather than doing nothing.
