...
class PromotionAdmin extends StatefulWidget {
  const PromotionAdmin({super.key, this.onChanged});
...
class _PromotionAdminState extends State<PromotionAdmin> {
  Future<List<SFPromotion>>? _future;
  ...
  void initState() {
    super.initState();
    if (_isAdmin) _future = PromotionService.listAdmin();
  ...
  void _authChanged() {
    ...
    setState(() {
      if (!_isAdmin) {
        ...
        _future ??= PromotionService.listAdmin();
      }
    });
  }
  ...
  Future<void> _reload() async {
    ...
    final request = PromotionService.listAdmin();
    setState(() {
      _future = request;
    });
    try {
  ...
  Future<void> _edit([SFPromotion? existing]) async {
    if (!_isAdmin) return;
    final changed = await Navigator.of(context).push<bool>(
      MaterialPageRoute(builder: (_) => _PromotionEditor(existing: existing)),
    );
  ...
  Future<void> _setActive(SFPromotion promotion, bool value) async {
    if (!_isAdmin || _busy.contains(promotion.id)) return;
    setState(() => _busy.add(promotion.id));
    try {
      await PromotionService.setActive(promotion, value);
      if (!mounted) return;
      widget.onChanged?.call();
      await _reload();
    } catch (error) {
  ...
  Future<void> _delete(SFPromotion promotion) async {
    if (!_isAdmin || _busy.contains(promotion.id)) return;
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<SFPromotion>>(
      future: _future,
      builder: (context, snapshot) {
        final ready =
            snapshot.connectionState == ConnectionState.done &&
            !snapshot.hasError;
        final promotions = snapshot.data ?? const <SFPromotion>[];
          ...
          child: ListView(
            ...
            children: [
              ...
              FilledButton.icon(
                ...
                label: Text(context.t('promo_add')),
              ),
              ...
              if (snapshot.connectionState != ConnectionState.done)
                ...
                SFStateView(
                  message: context.t('promo_admin_unavailable'),
  ...
  Widget _card(SFPromotion promotion) {
            ...
            Wrap(
              ...
              children: [
                OutlinedButton.icon(
                  ...
                  label: Text(context.t('promo_edit')),
                ),
                TextButton.icon(
...
class _PromotionEditorState extends State<_PromotionEditor> {
  ...
  Future<void> _discard(String url) async {
    try {
      await PromotionService.cleanupUploadedImage(url);
  ...
  Future<void> _pickImage() async {
    ...
    try {
      final url = await SFUpload.pickAndUploadImage(
        bucket: PromotionService.bucket,
        folder: 'banners',
        maxSize: 1600,
      );
      if (url == null) return;
  ...
  Future<void> _save() async {
    ...
    try {
      final saved = await PromotionService.save(
      ...
      _pendingImages.remove(saved.imageUrl);
      if (!mounted) return;
  ...
  Widget build(BuildContext context) => ListenableBuilder(
    ...
    builder: (context, _) => PopScope(
      ...
      child: Scaffold(
        appBar: SFTopBar(
          title: context.t(
            widget.existing == null ? 'promo_add' : 'promo_edit',
          ),
          showBack: true,
        ),
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        validator: (value) =>
                            SFPromotion.isValidTargetUrl(value?.trim() ?? '')
                            ? null
