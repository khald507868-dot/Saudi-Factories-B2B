...
import 'package:saudi_factories/widgets/video_promotion_admin.dart';
...
Widget _host() => I18nScope(
  ...
  child: MaterialApp(
    ...
    home: const Scaffold(body: VideoPromotionAdmin()),
  ),
);
...
void main() {
  ...
  setUp(() {
    ...
    row = {
      ...
      'title': 'عرض سابق',
      'image_url': _image,
      'video_url': _image.replaceAll('.png', '.mp4'),
      'logo_url': '',
      'target_url': '',
      'is_active': false,
      'sort_order': 0,
  ...
  testWidgets('الزائر والحساب العادي لا يطلبان بيانات إدارة الإعلانات', (
    ...
    expect(reads, 0);
    expect(find.text('إضافة إعلان فيديو'), findsNothing);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final add = tester.widget<FilledButton>(
        find.widgetWithText(FilledButton, 'إضافة إعلان فيديو'),
      );
  ...
  testWidgets('فشل الحفظ يحتفظ بالقيم وإعادة الحفظ تنجح وتغلق المحرر', (
    ...
    await openAdmin(tester);
    await tester.tap(find.text('تعديل إعلان الفيديو'));
    await tester.pumpAndSettle();
  ...
  testWidgets('الإعلان الجديد يحتاج صورة والرابط غير الآمن لا يرسل أي كتابة', (
    ...
    await openAdmin(tester);
    await tester.tap(find.text('إضافة إعلان فيديو'));
    await tester.pumpAndSettle();
